...
public class JpaNestedSetManager implements NestedSetManager {
    ...
    @Override
    public <T extends NodeInfo> List<Node<T>> fetchTreeAsList(Class<T> clazz, int rootId, int maxLevel) {
        Configuration config = getConfig(clazz);
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<T> cq = cb.createQuery(clazz);
        Root<T> queryRoot = cq.from(clazz);
        Predicate ge = cb.ge(queryRoot.<Number>get(config.getLeftFieldName()), 1);
        Predicate level = cb.lt(queryRoot.<Number>get(config.getLevelFieldName()), maxLevel);
        cq.where(ge, level);
        cq.orderBy(cb.asc(queryRoot.get(config.getLeftFieldName())));
        applyRootId(clazz, cq, rootId);
